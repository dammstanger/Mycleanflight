...
static void applyMulticopterPositionController(timeUs_t currentTimeUs)
{
    ...
    if (!bypassPositionController) {
        rcCommand[PITCH] = pidAngleToRcCommand(posControl.rcAdjustment[PITCH], imuConfig()->max_angle_inclination);
        rcCommand[ROLL] = pidAngleToRcCommand(posControl.rcAdjustment[ROLL], imuConfig()->max_angle_inclination);
    }
}
