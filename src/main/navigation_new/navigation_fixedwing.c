...
void applyFixedWingPitchRollThrottleController(navigationFSMStateFlags_t navStateFlags, timeUs_t currentTimeUs)
{
    ...
    if (isPitchAdjustmentValid && (navStateFlags & NAV_CTL_ALT)) {
        ...
        rcCommand[PITCH] = -pidAngleToRcCommand(pitchCorrection, imuConfig()->max_angle_inclination);
    }
    ...
    if (isRollAdjustmentValid && (navStateFlags & NAV_CTL_POS)) {
        ...
        rcCommand[ROLL] = pidAngleToRcCommand(rollCorrection, imuConfig()->max_angle_inclination);
    }
    ...
    if (posControl.flags.hasValidAltitudeSensor && posControl.actualState.pos.V.Z < navConfig()->general.land_slowdown_minalt) {
         ...
         */
        rcCommand[PITCH] = pidAngleToRcCommand(DEGREES_TO_DECIDEGREES(navConfig()->fw.land_dive_angle), imuConfig()->max_angle_inclination);
     }
#endif
}
...
void applyFixedWingEmergencyLandingController(void)
{
    ...
    rcCommand[ROLL] = pidAngleToRcCommand(failsafeConfig()->failsafe_fw_roll_angle, imuConfig()->max_angle_inclination);
    rcCommand[PITCH] = pidAngleToRcCommand(failsafeConfig()->failsafe_fw_pitch_angle, imuConfig()->max_angle_inclination);
