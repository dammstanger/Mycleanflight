...
bool isThrustFacingDownwards(void)
{
    ...
    return (getCosTiltAngle() >= 0.173648178f);
}
...
//        if (rcModeIsActive(BOXHOMERESET)) {
...
//    }
	...
	posControl.desiredState.pos.V.Z = desiredClimbRate;
	if(debugMode==DEBUG_NAV){debug[1] = posControl.desiredState.pos.V.Z; }
...
//        if (STATE(FIXED_WING)) {
//            if (rcModeIsActive(BOXNAVLAUNCH)) {     // FIXME: Only available for fixed wing aircrafts now
...
//        }
        ...
        if (posControl.flags.forcedRTHActivated || (rcModeIsActive(BOXNAVRTH) && canActivatePosHold && canActivateAltHold && STATE(GPS_FIX_HOME))) {
...
//        canActivateLaunchMode = rcModeIsActive(BOXNAVLAUNCH);
    }
...
{
    const bool navBoxModesEnabled = rcModeIsActive(BOXNAVRTH) || rcModeIsActive(BOXNAVWP) || rcModeIsActive(BOXNAVPOSHOLD);
//    const bool navLaunchComboModesEnabled = rcModeIsActive(BOXNAVLAUNCH) && (rcModeIsActive(BOXNAVRTH) || rcModeIsActive(BOXNAVWP));
    const bool navLaunchComboModesEnabled = (rcModeIsActive(BOXNAVRTH) || rcModeIsActive(BOXNAVWP));
...
{
//    if (posControl.flags.isGCSAssistedNavigationEnabled && !rcModeIsActive(BOXGCSNAV)) {
//        posControl.flags.isGCSAssistedNavigationReset = true;
//    }
//
//    posControl.flags.isGCSAssistedNavigationEnabled = rcModeIsActive(BOXGCSNAV);
    posControl.flags.isTerrainFollowEnabled = rcModeIsActive(BOXSURFACE);
}
