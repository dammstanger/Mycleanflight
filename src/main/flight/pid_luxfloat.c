        ...
        uint16_t max_angle_inclination, const rollAndPitchTrims_t *angleTrim, const rxConfig_t *rxConfig)
{
    ...
    for (int axis = 0; axis < 3; axis++) {
        ...
        if (axis == FD_YAW) {
            ...
            if (FLIGHT_MODE(ANGLE_MODE) || FLIGHT_MODE(HORIZON_MODE)||FLIGHT_MODE(BARO_MODE)) {		//baro模式也改为自水平控制
                ...
                const float errorAngle = constrain(2 * rcCommand[axis], -((int)max_angle_inclination), max_angle_inclination)
                        - attitude.raw[axis] + angleTrim->raw[axis];
#endif
                if (FLIGHT_MODE(ANGLE_MODE)||FLIGHT_MODE(BARO_MODE)) {			//baro模式也改为自水平控制,目标角速度使用与angle模式相同
